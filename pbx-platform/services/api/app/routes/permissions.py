...
#     # 1. 부모 메뉴(MENU) 존재 여부 확인
#     stmt = select(Permission).where(
#         Permission.code == data.menu_code, 
...
#     if menu:
#         # 이미 존재하면 이름 업데이트 및 다시 활성화(is_active=True)
...
#                 # 기존에 있는 액션이면 이름 업데이트 및 다시 활성화
#                 db_actions[act_data.code].name = act_data.name
#                 db_actions[act_data.code].is_active = True
#             else:
#                 # 없는 액션이면 신규 추가
...
#                     parent_id=menu.id,
#                     is_active=True # 신규 액션 활성화
#                 )
#                 db.add(new_action)

#     # 3. UI에서 제외된 액션은 비활성화 처리 (소프트 딜리트)
...
#             "status": "success",
#             "menu_id": menu.id,
#             "action_count": len(data.actions) if data.actions else 0
#         }
